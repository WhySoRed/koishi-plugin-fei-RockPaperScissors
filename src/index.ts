...
export const Config: Schema<Config> = Schema.object({
  rpsTime: Schema.number()
    .default(10000)
    .description("剪刀石头布游戏倒计时（毫秒）"),
  rpsWaitTime: Schema.number()
    .default(60000)
    .description("剪刀石头布等待对方同意的时间（毫秒）"),
  rpsPreparedTime: Schema.number()
    .default(3000)
    .description("剪刀石头布游戏准备时间（毫秒）"),
});
...
export function apply(ctx: Context, config: Config) {
  ...
  class RpsTemp {
    ...
    constructor(player1Id: string, player2Id: string, player1Name: string) {
  ...
  ctx
    ...
    .action(async ({ session }, message) => {
      ...
      if (h.select(message, "at").length != 1) return "请@一个人呀";
      else {
        //艾特自己
        if (h.select(message, "at")[0].attrs.id === session.event.user.id)
          return "你要和自己玩吗...那怎么行";
        //艾特机器人
        if (h.select(message, "at")[0].attrs.id === session.bot.selfId) {
          ...
          if (userInput === null) {
            ...
            userInput === "rock" ||
            userInput === "Rock" ||
            /[(👊)(👊🏻)(👊🏼️)(👊🏽️)(👊🏾️)(👊🏿️)(✊)(✊🏻)(✊🏼️)(✊🏽️)(✊🏾️)(✊🏿️)(🤜)(🤜🏻)(🤜🏼️)(🤜🏽️)(🤜🏾️)(🤜🏿️)(🤛)(🤛🏻)(🤛🏼️)(🤛🏽️)(🤛🏾️)(🤛🏿️)]/.test(
              userInput
            )
          )
            return "我出 布~你输啦";
        ...
        const rps = (rpsTemp[session.cid] = new RpsTemp(
          session.event.user.id,
          h.select(message, "at")[0].attrs.id,
          nicknameOn
            ? ((await ctx.nickname.getNick(session)) as string)
            : session.event.user.name
        ));
        rps.gamePrepared = true;
        rps.endWait = ctx
          .channel(session.event.channel.id)
          .on("message", async (session) => {
            ...
            if (session.event.user.id == rps.player[1].id) {
              ...
              session.send(
                h.at(rps.player[0].id) +
                  " " +
                  h.at(rps.player[1].id) +
                  " 游戏就要开始咯~请做好准备，" +
                  config.rpsPreparedTime / 1000 +
                  "秒后游戏将会开始"
              );
            }
            //自己同意
            ...
            //其他人同意
            else session.send(h.at(session.event.user.id) + " 你不要掺和啦...");
          });
      ...
      return (
        ...
        " " +
        (nicknameOn
          ? await ctx.nickname.getNick(
              session,
              h.select(message, "at")[0].attrs.id
            )
          : "") +
        '~要玩剪刀石头布吗？如果同意的话，请发送"同意"~'
      );
    }).usage(`
剪刀石头布 @某人 或
石头剪刀布 @某人
...
可以慢出也可以改出，但不出会输哟`);
  ...
  async function startGame(session: Session) {
    ...
    rps.gameBegun = true;
    rps.player[1].name = nicknameOn
      ? ((await ctx.nickname.getNick(session)) as string)
      : session.event.user.name;

    rps.endListion = ctx.on("message", (session) => {
      if (
        session.event.user.id == rps.player[0].id ||
  ...
  async function changeChoice(session: Session) {
    const rps = rpsTemp[session.cid];
    const player =
      rps.player[rps.player[0].id === session.event.user.id ? 0 : 1];
    if (
      session.content === "剪刀" ||
    ...
    else session.send(player.name + " 改成了 " + player.choice);
  }
  //结算
  async function settle(session: Session) {
    ...
    } else if (rps.player[0].choice === undefined) winer = true;
    else if (rps.player[1].choice === undefined) winer = false;
    ...
    session.send(
      "游戏时间到~\n" +
        rps.player[0].name +
        "的结果是 " +
        (rps.player[0].choice === undefined
          ? "啥也没出"
          : rps.player[0].choice) +
        "\n" +
        rps.player[1].name +
        "的结果是 " +
        (rps.player[1].choice === undefined
          ? "啥也没出"
          : rps.player[1].choice) +
        "\n" +
        (winer === undefined
          ? "平局"
          : winer
          ? rps.player[1].name + " 赢了"
          : rps.player[0].name + " 赢了") +
        "~"
    );
    if (winer !== undefined) {
      ...
      const count =
        (
          await ctx.database.get("rpsWinCount", {
            cid,
            uid,
            loserId: rps.player[+!winer].id,
          })
        )[0]?.count || 0;
      await ctx.database.upsert("rpsWinCount", [
        {
          cid,
          uid,
          loserId: rps.player[+!winer].id,
          loserName: rps.player[+!winer].name,
          count: count + 1,
        },
      ]);
    }
  ...
  ctx
    ...
    .action(async ({ session }, page) => {
      ...
      const winRecord = await ctx.database
        .select("rpsWinCount")
        .where({ uid })
        .groupBy("loserId", {
      ...
      if (winRecord.length === 0) {
        ...
        const winText = JSON.parse(
          await fs_1.readFileSync(path_1.join(__dirname, "/RpsShowText.json"))
        ).winText;
        return (
          `你的剪刀石头布胜利记录：\n=================\n` +
          (
            await Promise.all(
              winRecord.map(async (record) => {
                const loser = nicknameOn
                  ? await ctx.nickname.getNickGiven(session, record.loserId)
                  : record.loserName;
                const count = record.count;
                const randomWinText =
                  winText[Math.floor(Math.random() * winText.length)];
                return randomWinText
